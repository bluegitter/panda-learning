...
def article(cookies, a_log, wechat_users, each):
    if each[0] < 6 or each[3] < 8:
        ...
        if try_count < 10:
            print("文章学习完成")
            itchat.send("文章学习完成", toUserName=wechat_users[0]["UserName"]);
        else:
            print("文章学习出现异常，请检查用户名下a_log文件记录数")
        driver_article.quit()
    else:
        print("文章之前学完了")
        itchat.send("文章之前学完了", toUserName=wechat_users[0]["UserName"]);


def video(cookies, v_log, wechat_users, each):
    if each[1] < 6 or each[4] < 10:
        ...
        if try_count < 10:
            print("视频学习完成")
            itchat.send("视频学习完成", toUserName=wechat_users[0]["UserName"]);
        else:
            print("视频学习出现异常，请检查用户名下v_log文件记录数")
        driver_video.quit()
    else:
        print("视频之前学完了")
        itchat.send("视频之前学完了", toUserName=wechat_users[0]["UserName"]);
...
if __name__ == '__main__':
    ...
    article_thread = threads.MyThread("文章学习", article, cookies, a_log, wechat_users, each, lock=lock)
    video_thread = threads.MyThread("视频学习", video, cookies, v_log, wechat_users, each, lock=lock)
